...
本仓库提供两个分支,内容来自同一个 pipeline 的不同发展阶段。

| 分支 | 是什么 | 适合谁 |
|---|---|---|
| **`main`(当前)** | 重构精简版 · 机制层经过 10 wave 重构整理后的版本 | 想**跑起来体验** / 想看清架构骨架 / 想读源码学习 |
| [**`full-arch`**](https://github.com/duliangkuan/fengyun-publish/tree/full-arch) | 累积复杂版 · 18 个 Phase 一路堆叠出来的完整原始态 | 想**深度二次开发** / 想要更全的子系统 / 不在乎冗余路径 |

> 风云本人 A/B 实测过,**单篇产出效果两个版本差别不大** — 机制层重构本来就是单篇照不出来的事。
> 所以普通使用者建议 `main` 就好。
> 只有打算自己魔改 / 加新功能 / 想看一个 AI 公众号 pipeline 真实演化痕迹的人,才需要切到 `full-arch`。

切换方式:`git checkout full-arch` 或在 GitHub 页面上方分支下拉里选。
